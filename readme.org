  ...
  (setf org-capture-templates
        '(("c" "Clock into new")
          ...
          ("cm" "Meeting" entry (file+datetree "~/org/agenda/meetings.org")
           "* %?\n+ *Agenda*:\n+ *Attendees*:" :clock-in t :clock-keep t)
...
...coming soon.
* IRC
I want to use my authinfo file to store all my IRC accounts. This grabs all the IRC connections stored in it and connects to each one.
#+BEGIN_SRC emacs-lisp
  (defun irc-connect ()
    (interactive)
    (require 'auth-source)
    (let ((auth (auth-source-search :port "irc" :requires '(user secret host))))
      (dolist (login auth)
