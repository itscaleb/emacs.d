...
#+END_SRC
* Nodejs Config
A quick hack to get nodejs and a few packages running in nix-repl
#+BEGIN_SRC emacs-lisp
(defun nix-node-command (packages)
  (car
   (last
    (split-string
     (shell-command-to-string
      "nix-shell -p nodejs --run 'npm install ramda underscore'; which node")
     "[\n]+" t))))

(setq nodejs-repl-command #'nix-node-command)
#+END_SRC
* Racket
#+BEGIN_SRC emacs-lisp
